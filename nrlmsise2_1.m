...
ModelDir = fullfile(RootDir,'nrlmsise2.1');
...
if ~exist(fullfile(RootDir,'mex_nrlmsise2_1'),'file')
    OldDir = pwd;
    cd(ModelDir);
    try
        mex('-compatibleArrayDims','-DO3','-Dstatic','-DfPIC', ...
            '-Dffast-math','-Dmarch=native', ...
            '-output',fullfile(RootDir,'mex_nrlmsise2_1'), ...
            fullfile(ModelDir,'msis_constants.F90'), ...
            fullfile(ModelDir,'msis_utils.F90'), ...
            fullfile(ModelDir,'msis_init.F90'),...
            fullfile(ModelDir,'msis_gfn.F90'), ...
            fullfile(ModelDir,'msis_tfn.F90'), ...
            fullfile(ModelDir,'msis_dfn.F90'), ...
            fullfile(ModelDir,'msis_calc.F90'),...
            fullfile(RootDir,'mex_nrlmsise2_1.F'))
        cd(OldDir)
    catch err
        cd(OldDir)
        rethrow(err)
...
if Update % update apf107.dat file in iri directory
    read_apf(fullfile(RootDir,'apf107.dat'),true);
end
...
APF = read_apf(fullfile(RootDir,'apf107.dat'));
